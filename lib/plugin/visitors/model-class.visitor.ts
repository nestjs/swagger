...
} from '../../services/decorators-properties.js';
...
export class ModelClassVisitor extends AbstractFileVisitor {
  ...
  private createTypePropertyAssignments(
    ...
    if (node) {
      ...
      if (ts.isUnionTypeNode(node)) {
        ...
        if (remainingTypes.length === 1) {
          ...
          if (isEnumType) {
            // For enums and string literal unions, skip returning a "type" property.
            // Only append { nullable: true } if the union contained `null`.
            // The enum/enum-array metadata will be added by createEnumPropertyAssignment().
            return isNullable
              ? [
                  factory.createPropertyAssignment(
                    'nullable',
    ...
    // String literal union types like `'a' | 'b'` or `'a' | 'b' | null` —
    // skip the `type` property; createEnumPropertyAssignment() emits `enum: [...]`.
    // Emit `nullable: true` here so createEnumPropertyAssignment doesn't have to.
    const stringLiteralUnion = getStringLiteralUnionValues(type);
    if (stringLiteralUnion) {
      if (
        stringLiteralUnion.isNullable &&
  ...
  createEnumPropertyAssignment(
    ...
    if (stringLiteralUnion) {
      ...
      // nullable is handled by createTypePropertyAssignments() for all paths.
    ...
    if (
      options.autoFillEnumName &&
      !hasPropertyKey('enumName', existingProperties)
    ) {
      const fullTypeName = typeReferenceDescriptor.typeName;
      // getText() may return a fully-qualified name like `import("./file.js").EnumName`;
