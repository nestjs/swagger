...
export function getTypeReferenceAsString(
  ...
  try {
    ...
    if (
      ...
      isNullableOrUndefinedEnumUnion(type, typeChecker)
    ) {
 ...
 * @param typeChecker
 */
export function isNullableOrUndefinedEnumUnion(
  type: ts.Type,
  typeChecker: ts.TypeChecker
): ts.Type {
  if (type.isUnionOrIntersection() && !isEnum(type)) {
    ...
    const types = type.types.filter((type) => {
    ...
    const isParentSymbolEqual = types.every((item) => {
      if (!item.symbol) {
        return false;
      }
      if (
