 ...
 * TypeScript union.
 *
 * Use the token in Swagger decorators and `InferUnionApiSchema` for application
 * types. It does not validate or transform values. Members must describe mutually
 * exclusive schemas: OpenAPI `oneOf` requires exactly one matching member.
 *
 * @example
 * ```ts
...
>(
  ...
  const oneOf: Array<SchemaObject | ReferenceObject> = [];
  for (const member of options.oneOf) {
    ...
    if (!oneOf.some((existing) => isEqual(existing, schema))) {
      oneOf.push(schema);
    }
  }
  ...
  const models = [...modelsByPath.values()];
  assertValidModelGraph(models, modelsByPath, unionPath, options.name);
...
function createModelReference(
  ...
  return registerModelReference(model, modelsByPath, unionPath, unionName);
}

function registerModelReference(
  model: Function,
  modelsByPath: Map<string, Function>,
  unionPath: string,
  unionName: string
...
function assertValidModelGraph(
  models: Function[],
  modelsByPath: Map<string, Function>,
  unionPath: string,
  unionName: string
  ...
  const visit = (model: Function) => {
    if (visited.has(model)) {
      return;
    }
    visited.add(model);
    for (const extraModel of exploreGlobalApiExtraModelsMetadata(
      model as Type<unknown>
    )) {
      registerModelReference(extraModel, modelsByPath, unionPath, unionName);
      visit(extraModel);
    }
  };
